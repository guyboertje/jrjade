...
public class TagNode extends AttributedNode {
    ...
    public void execute(ScriptWriter writer, JadeModel model, JadeTemplate template) throws JadeCompilerException {
        ...
        writer.concat("<");
        writer.concat(name);
        appendAttributes(model, template, writer);
        if (isTerse(template)) {
            writer.concat(">");
            return;
        }
...
//          ExpressionNode
    ...
    private void appendAttributes(JadeModel model, JadeTemplate template, ScriptWriter writer) {
        Map<String, Object> mergedAttributes = mergeInheritedAttributes(model);
        for (Map.Entry<String, Object> entry : mergedAttributes.entrySet()) {
            try {
                appendAttributeString(entry.getKey(), entry.getValue(), model, template, writer);
            } catch (ExpressionException e) {
                throw new JadeCompilerException(this, template.getTemplateLoader(), e);
            }
        }
    ...
    private void appendAttributeString(String name, Object attribute, JadeModel model, JadeTemplate template, ScriptWriter writer) throws ExpressionException {
        String value = null;
        if (attribute instanceof String) {
            value = getInterpolatedAttributeValue(name, attribute, model, template, writer);
        } else if (attribute instanceof Boolean) {
            if ((Boolean) attribute) {
                value = name;
            } else {
            ...
            String key = ((ExpressionString) attribute).getValue();
            value = (String) model.get(key);
            if (value == null) {
                value = writer.wrapForEscapedAttribute(key);
            }
        } else {
            return;
        }
...
//    private 
    ...
    private String getInterpolatedAttributeValue(String name, Object attribute, JadeModel model, JadeTemplate template, ScriptWriter writer)
            throws JadeCompilerException {
        if (!preparedAttributeValues.containsKey(name)) {
            preparedAttributeValues.put(name, Utils.prepareInterpolate((String) attribute, false));
        }
        List<Object> prepared = preparedAttributeValues.get(name);
        try {
            return Utils.interpolate(prepared, writer);
        } catch (ExpressionException e) {
            throw new JadeCompilerException(this, template.getTemplateLoader(), e);
        }
    }    
